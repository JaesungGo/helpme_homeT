...
class DetectPullUp : AppCompatActivity() {
    ...
    private var firstPullUpTime = 0L
    private var lastPullUpTime = 0L
    private var isPullUpting = false
    private var isPullUpSitting = false
    private var isPullUpStanding = false
    private var pull_ups = 0
    private val wrongList = mutableListOf<Int>()
    private var wrongCount = 0
    private var wrongPullUp = 0
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.activity_detect_pull_up)
        get_permissions()
...
//                }
                ...
                if (60 <= LangleInDegrees && 60 <= RangleInDegrees &&
                    ...
                    isPullUpting = true
                    firstPullUpTime = currentTime // 처음 앉은 시간
                ...
                if (isPullUpting && currentTime - firstPullUpTime >= COOLDOWN_TIME_MS
                    && 10 >= LangleInDegrees && 10 >= RangleInDegrees ) // 스쿼트 후 앉지 않고 일어섰을 때
                {
                    pull_ups++ // pull_up 횟수 증가
                    isPullUpting = false
                }
                    ...
                    30 >= RangleInDegrees && RangleInDegrees >10 ) // 주저 앉았을때
                {
                    ...
                    isPullUpting = false
                    isPullUpStanding =false
                    isPullUpSitting = true
                    if (wrongList.isEmpty() || wrongList.last() != 1) {
                        wrongList.add(1)
                    }
                }

                if (isPullUpSitting && 10 >= LangleInDegrees && 10 >= RangleInDegrees ) // 앉은 후 일어섰을때
                {
                    isPullUpSitting = false
                    isPullUpStanding = true
                    paint.color = Color.WHITE
                    wrongList.add(0)
                }

                // pull_up 횟수 텍스트 뷰 업데이트
                val squatsTextView = findViewById<TextView>(R.id.pullupTextView)
                squatsTextView.text = "현재 pullup 개수: $pull_ups"
                val squatsTextView2 = findViewById<TextView>(R.id.pullupTextView2)
                squatsTextView2.text = "틀린 pullup 개수 : $wrongPullUp"
...
//    }
    ...
    override fun onBackPressed() {
        ...
        val activaValue2 = wrongPullUp
        val squatsRef = userRef.child("Check2").child(today).child("PullUp").child(today2)
        val squatsRef2 = userRef.child("Check2").child(today).child("wrongPullUp").child(today2)
